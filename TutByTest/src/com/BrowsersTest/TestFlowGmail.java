...
public class TestFlowGmail {
	...
	public static void testFlowSteps(WebDriver driver) throws InterruptedException {
		...
		WebElement emailField = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
				.until(ExpectedConditions.presenceOfElementLocated(By.id("Email")));

		emailField.sendKeys("WebDriverTest01");

		WebElement nextbutton = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
		...
		WebElement sendButton = (new WebDriverWait(driver, TutByAndGmailTest.WAITING)).until(
				ExpectedConditions.presenceOfElementLocated(By.xpath("//div[@role='button' and text()='Send']")));

		sendButton.click();

		WebElement viewMessage = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
		...
		WebElement inboxButton = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
				.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span/a")));

		inboxButton.click();

		WebElement newMail = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
				.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//b[1][text()='Hello world']")));

		newMail.isDisplayed();

		WebElement firstMail = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
				.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//td[4]/div[2]")));

		firstMail.click();

		WebElement messageText = (new WebDriverWait(driver, TutByAndGmailTest.WAITING))
